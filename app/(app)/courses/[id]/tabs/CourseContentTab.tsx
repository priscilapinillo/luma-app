...
export default function CourseContentTab({ cursoId }: { cursoId: string }) {
  ...
  async function cargarModulos() {
    try {
      ...
      if (!mods) return
      const { data: lecs } = await supabase.from('lessons').select('*')
        .in('module_id', mods.map(m => m.id)).order('orden')
      setModulos(mods.map(m => ({ ...m, lecciones: lecs?.filter(l => l.module_id === m.id) || [] })))
  ...
  async function guardarLeccion(moduleId: string) {
    ...
    try {
      ...
      const datos = {
        ...
        duracion_min: formLeccion.duracion_min ? Number(formLeccion.duracion_min) : null,
        es_preview: formLeccion.es_preview,
      }
      if (editandoLeccion) {
        await supabase.from('lessons').update(datos).eq('id', editandoLeccion.id)
      } else {
        await supabase.from('lessons').insert({ ...datos, module_id: moduleId, orden: lecciones.length })
      }
      await cargarModulos()
      setModalLeccion(null)
      setFormLeccion({ titulo: '', tipo: 'video', contenido_url: '', contenido_texto: '', duracion_min: '', es_preview: false })
          ...
          {moduloAbierto === m.id && (
            <>
              {m.lecciones?.map(l => (
                <div key={l.id} className="lec-item">
                  ...
                  <div className="mod-actions">
                    <div className="icon-btn" onClick={() => { setEditandoLeccion(l); setFormLeccion({ titulo: l.titulo, tipo: l.tipo, contenido_url: l.contenido_url || '', contenido_texto: l.contenido_texto || '', duracion_min: l.duracion_min || '', es_preview: l.es_preview }); setModalLeccion(m.id) }}>
                      <Edit2 size={10} color="var(--text-muted)"/>
                    </div>
              ...
              <div style={{padding:'10px 16px',borderTop:'0.5px solid var(--border-light)'}}>
                <button onClick={() => { setEditandoLeccion(null); setFormLeccion({ titulo: '', tipo: 'video', contenido_url: '', contenido_texto: '', duracion_min: '', es_preview: false }); setModalLeccion(m.id) }}
        ...
        <div className="modal-overlay" onClick={() => setModalLeccion(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="field">
              ...
              <input type="number" min="0" value={formLeccion.duracion_min}
