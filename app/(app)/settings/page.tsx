...
import { User, Shield, Database, Settings, Camera, Eye, EyeOff, Download, Trash2, LogOut, X } from 'lucide-react'
...
export default function AjustesPage() {
  ...
  const [passForm, setPassForm] = useState({
    actual: '', nueva: '', confirmar: '', showActual: false, showNueva: false,
  })
  ...
  async function cambiarPassword() {
    ...
    setPassExito(true)
    setPassForm({ actual: '', nueva: '', confirmar: '', showActual: false, showNueva: false })
  ...
  async function exportarCSV(tipo: 'pacientes' | 'finanzas') {
    ...
    if (tipo === 'pacientes') {
      ...
      if (!data) return
      const csv = [
        'Nombre,Apellido,Celular,Email,Cumpleaños,Contexto',
        ...data.map(p => `${p.nombre},${p.apellido},${p.celular||''},${p.email||''},${p.fecha_nacimiento||''},${(p.contexto_general||'').replace(/,/g,' ')}`)
      ].join('\n')
      ...
      if (!data) return
      const csv = [
        'Fecha,Hora,Servicio,Precio,Estado Pago,Estado Sesion',
        ...data.map(s => `${s.fecha?.split('T')[0]||''},${s.hora||''},${s.servicio_nombre||''},${s.precio||0},${s.estado_pago||''},${s.estado_sesion||''}`)
      ].join('\n')
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box}
        .sw{height:100vh;display:grid;grid-template-columns:220px 1fr;font-family:'Inter',sans-serif;background:#F4F2FF;overflow:hidden}
        .s-nav{background:white;border-right:0.5px solid #EDE9FF;padding:20px 12px;display:flex;flex-direction:column;gap:2px;box-shadow:2px 0 8px rgba(139,92,246,0.04)}
        .s-nav-title{font-size:16px;font-weight:800;color:#1A1035;padding:0 10px;margin-bottom:16px;letter-spacing:-0.3px}
        .s-nav-item{display:flex;align-items:center;gap:9px;padding:9px 10px;border-radius:10px;font-size:12px;font-weight:500;color:#7C6BAA;cursor:pointer;transition:all 0.15s;border:none;background:none;font-family:inherit;width:100%;text-align:left}
        .s-nav-item:hover{background:#F0EBFF;color:#3B0F8C}
        .s-nav-item.active{background:#F0EBFF;color:#3B0F8C;font-weight:600}
        .s-nav-div{border:none;border-top:0.5px solid #EDE9FF;margin:6px 0}
        .s-content{overflow-y:auto;padding:24px 28px}
        .s-section-title{font-size:18px;font-weight:800;color:#1A1035;letter-spacing:-0.5px;margin-bottom:4px}
        .s-section-sub{font-size:12px;color:#A99CC4;margin-bottom:24px}

        .s-card{background:white;border-radius:16px;padding:20px 22px;border:0.5px solid #EDE9FF;box-shadow:0 2px 12px rgba(139,92,246,0.07);margin-bottom:16px}
        .s-card-title{font-size:13px;font-weight:700;color:#1A1035;margin-bottom:14px;display:flex;align-items:center;gap:7px}
        .s-card-title svg{opacity:0.6}

        .avatar-wrap{display:flex;align-items:center;gap:16px;margin-bottom:20px}
        .avatar-img{width:72px;height:72px;border-radius:50%;object-fit:cover;border:2px solid #EDE9FF}
        ...
        .avatar-info{}
        .avatar-name{font-size:16px;font-weight:700;color:#1A1035;margin-bottom:2px}
        .avatar-spec{font-size:12px;color:#A99CC4}
        .avatar-btn{display:flex;align-items:center;gap:6px;padding:7px 14px;background:#F0EBFF;color:#7C3AED;border:none;border-radius:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;margin-top:8px;transition:all 0.15s}
        .avatar-btn:hover{background:#EDE8FF}
        ...
        .field-hint{font-size:10px;color:#A99CC4;margin-top:2px}
        ...
        .pass-field label{font-size:11px;font-weight:600;color:#1A1035;display:block;margin-bottom:5px}
        .pass-input-wrap{position:relative}
        .pass-input{width:100%;padding:9px 38px 9px 11px;border-radius:10px;border:0.5px solid #E2D9FF;font-size:13px;font-family:inherit;color:#1A1035;background:#FAFAFF;outline:none}
        .pass-input:focus{border-color:#8B5CF6;box-shadow:0 0 0 3px rgba(139,92,246,0.08)}
        .pass-eye{position:absolute;right:11px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#A99CC4;display:flex;align-items:center}
        ...
        .subs-card{background:linear-gradient(135deg,#1A1035,#3B1F7A);border-radius:16px;padding:20px 22px;margin-bottom:16px}
        ...
        .subs-plan{font-size:20px;font-weight:800;color:white;letter-spacing:-0.5px;margin-bottom:4px}
        ...
        .subs-trial-fill{height:100%;background:linear-gradient(90deg,#A78BFA,#C084FC);border-radius:6px;transition:width 0.5s}
        .subs-trial-text{font-size:11px;color:rgba(255,255,255,0.5)}

        .danger-zone{border:0.5px solid #FECACA;border-radius:12px;padding:16px 18px;background:#FFF5F5}
        .danger-title{font-size:12px;font-weight:700;color:#EF4444;margin-bottom:10px}
        .danger-btn{display:flex;align-items:center;gap:7px;padding:8px 14px;border-radius:9px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;border:0.5px solid #FECACA;background:white;color:#EF4444;transition:all 0.15s;width:100%;margin-bottom:8px}
        ...
        .export-card{background:#F8F6FF;border-radius:12px;padding:16px;border:0.5px solid #EDE9FF;text-align:center;cursor:pointer;transition:all 0.15s}
        .export-card:hover{background:#EDE8FF;border-color:#C4B8E8;transform:translateY(-1px)}
        .export-icon{width:36px;height:36px;border-radius:10px;background:#EDE8FF;display:flex;align-items:center;justify-content:center;margin:0 auto 10px}
        .export-title{font-size:12px;font-weight:600;color:#1A1035;margin-bottom:3px}
        .export-sub{font-size:10px;color:#A99CC4}

        .pref-row{display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:0.5px solid #EDE9FF}
        .pref-row:last-child{border-bottom:none}
        .pref-label{font-size:13px;font-weight:500;color:#1A1035}
        .pref-sub{font-size:11px;color:#A99CC4;margin-top:2px}
        .pref-select{padding:6px 10px;border-radius:8px;border:0.5px solid #E2D9FF;font-size:12px;font-family:inherit;color:#1A1035;background:white;outline:none;cursor:pointer}
        .pref-select:focus{border-color:#8B5CF6}
        ...
        .confirm-box{background:white;border-radius:20px;padding:24px;width:360px;box-shadow:0 32px 80px rgba(100,60,200,0.25)}
        .confirm-title{font-size:15px;font-weight:700;color:#1A1035;margin-bottom:8px}
        .confirm-text{font-size:13px;color:#6B5B8A;line-height:1.6;margin-bottom:20px}
        .confirm-btns{display:flex;gap:8px}
        .confirm-cancel{flex:1;padding:10px;border-radius:10px;border:0.5px solid #E2D9FF;background:white;font-size:13px;cursor:pointer;font-family:inherit;color:#6B5B8A}
              ...
              <div className="subs-card">
                ...
                <div className="subs-info">
                  {suscripcion?.status === 'trial'
                    ? `${diasTrial} días restantes de prueba gratuita`
                    : 'USD 7/mes · Facturación mensual'}
                </div>
                {suscripcion?.status === 'trial' && (
                  <>
            ...
            <>
              ...
              <div className="s-card">
                ...
                <div className="pass-field">
                  ...
                  <div className="pass-input-wrap">
                    <input className="pass-input" type={passForm.showActual ? 'text' : 'password'}
                      placeholder="Repetí la contraseña" value={passForm.confirmar}
                      onChange={e => setPassForm({...passForm, confirmar: e.target.value})}/>
                    <button className="pass-eye" onClick={() => setPassForm({...passForm, showActual: !passForm.showActual})}>
                      {passForm.showActual ? <EyeOff size={14}/> : <Eye size={14}/>}
                    </button>
                  </div>
                </div>
              ...
              <div className="s-card">
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                  <div style={{background:'#F8F6FF',borderRadius:'10px',padding:'12px',border:'0.5px solid #EDE9FF'}}>
                    <div style={{fontSize:'10px',fontWeight:'700',color:'#A99CC4',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:'5px'}}>Email de cuenta</div>
                    <div style={{fontSize:'13px',fontWeight:'600',color:'#1A1035'}}>{email}</div>
                  </div>
                  <div style={{background:'#F8F6FF',borderRadius:'10px',padding:'12px',border:'0.5px solid #EDE9FF'}}>
                    <div style={{fontSize:'10px',fontWeight:'700',color:'#A99CC4',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:'5px'}}>Último acceso</div>
                    <div style={{fontSize:'13px',fontWeight:'600',color:'#1A1035'}}>
                      {ultimoAcceso ? new Date(ultimoAcceso).toLocaleDateString('es-AR',{day:'numeric',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'}) : '—'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="s-card">
                ...
                <button className="save-btn" style={{background:'#F0EBFF',color:'#7C3AED',boxShadow:'none'}} onClick={cerrarSesion}>
          ...
          {tab === 'datos' && (
            <>
              ...
              <div className="s-card">
                ...
                <p style={{fontSize:'13px',color:'#6B5B8A',lineHeight:'1.7',margin:0}}>
                  Todos tus datos se almacenan de forma segura en Supabase con cifrado en reposo. Cada terapeuta tiene sus datos completamente aislados mediante Row Level Security. Podés exportar o eliminar tu información en cualquier momento.
                </p>
              </div>
            </>
          )}
        ...
        <div className="confirm-overlay" onClick={() => setConfirmEliminar(false)}>
          <div className="confirm-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="confirm-text">
              Esta acción es <strong>permanente e irreversible</strong>. Se eliminarán todos tus pacientes, sesiones, pagos y datos asociados. No hay vuelta atrás.
            </div>
